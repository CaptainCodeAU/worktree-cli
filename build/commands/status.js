...
import { getWorktrees, isWorktreeClean } from "../utils/git.js";
/**
 * Get detailed git status for a worktree
 */
async function getWorktreeGitStatus(path) {
    try {
        ...
        try {
            ...
            const { stdout: upstreamBranch } = await execa("git", ["-C", path, "rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{upstream}"], { reject: false });
            if (upstreamBranch && upstreamBranch.trim()) {
                hasUpstream = true;
                // Get ahead/behind counts
                const { stdout: revList } = await execa("git", ["-C", path, "rev-list", "--left-right", "--count", "HEAD...@{upstream}"], { reject: false });
 ...
 */
function formatWorktreeStatus(wt, status) {
    const parts = [];
    ...
    try {
        ...
        for (const wt of worktrees) {
            try {
                ...
                console.log(chalk.cyan.bold(wt.branch || '(unknown)') +
                    chalk.gray(` → ${wt.path}`) +
